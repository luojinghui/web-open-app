...
export default class OpenApp {
	...
	public open() {
		...
		const { weixinVersion, isIOS, isWeixin, isAndroid, isChrome, isSafari, isFirefox } = this.systemInfo;
	...
	public timeoutEvent() {
		...
		const pageChange = e => {
			const isHide = document[property] || e.hidden || document.visibilityState == 'hidden';
		...
		if (!(isIOS && isSafari && this.options.iosUseScheme)) {
			...
			input.setAttribute('style', 'opacity: 0; position: fixed; top: 0; left: -999px; width: 1px; height: 1px');
